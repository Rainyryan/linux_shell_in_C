...
static int exec(char*** parsed, int fd_in, int fd_out) {
    ...
    int fd_reg = fd_in;
    if (!**parsed)
        return 1;        // bugfix: blank space inputs would cause Segmentation Fault, since *cmd isn't NULL but **cmd is NULL
    if (!*(parsed + 1))  // if there's only one command, check if it is builtin (for cd and exec fork() would not work)
        if (!exec_builtin((*parsed)[0], *parsed))
            return 0;
    while (*parsed) {
        if (pipe(fd) == -1)
            perror("pipe"), exit(1);
            ...
            if (*(parsed + 1))
                dup2(fd[1], 1);  // if there's a following command, redirect all the output to write end of pipe (from stdout)
            else
                dup2(fd_out, 1);
            close(fd[0]);
